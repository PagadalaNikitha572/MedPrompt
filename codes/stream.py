...
if st.button("Proceed") and clinical_text.strip():
    ...
    if not filtered_entities:
        ...
        st.subheader("Step 2: Entity Definitions")
        definitions = explain_entities(filtered_entities)
        for definition in definitions:
            st.markdown(f"{definition}")

        st.markdown("---")
        st.subheader("Step 3: Structured Information Extraction")
